            ...
            for i in lst[:-ind]:
                ...
                for result in results:
                    ...
                    if '_' in result:
                        ...
                        has_entered = True
                test_file = ocr_space_file(filename=i, api_key='K85194065988957')
                ...
                if test_file['ParsedResults'][0]['TextOverlay']['Lines']:
                    for a in test_file['ParsedResults'][0]['TextOverlay']['Lines']:
                        a = a['LineText'].split()
                        for j in a:
                            print(len(j) > 12, j[:12].isdigit(), j[13:].isdigit(), data2[0] == '', j)
                            if len(j) >= 12 and j[:12].isdigit() and not j[13:].isdigit() and data2[0] == '':
                                print('Suborder:', j[:12])
                                data2[0] = j[:12]
                                data_path['waybill'].append(i)
                                has_entered = True
                            ...
                            if (10 == len(j) and j.isdigit()) or (12 < len(j) and j.isdigit()) and \
                                    data2[1] == '' or j.endswith('FPL'): # for Ecom express, for Xpress bees and Delhivery and Shadowfax
                                print('Awb:', j)
                                data_path['waybill'].append(i)
                                data2[1] = j
                                has_entered = True
